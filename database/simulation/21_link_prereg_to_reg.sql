...
BEGIN
  ...
  IF has_reg_id THEN
    WITH samp AS (
      SELECT reg_id FROM regprc.registration WHERE status_code='CREATED' ORDER BY random() LIMIT v_prog
    )
    UPDATE regprc.registration r SET status_code='IN_PROGRESS'
    FROM samp s WHERE r.reg_id = s.reg_id;

    WITH samp AS (
      SELECT reg_id FROM regprc.registration WHERE status_code='IN_PROGRESS' ORDER BY random() LIMIT (v_prog*9/10)
    )
    UPDATE regprc.registration r SET status_code='APPROVED'
    FROM samp s WHERE r.reg_id = s.reg_id;
  ELSE
    WITH samp AS (
      SELECT id FROM regprc.registration WHERE status_code='CREATED' ORDER BY random() LIMIT v_prog
    )
    UPDATE regprc.registration r SET status_code='IN_PROGRESS'
    FROM samp s WHERE r.id = s.id;

    WITH samp AS (
      SELECT id FROM regprc.registration WHERE status_code='IN_PROGRESS' ORDER BY random() LIMIT (v_prog*9/10)
    )
    UPDATE regprc.registration r SET status_code='APPROVED'
    FROM samp s WHERE r.id = s.id;
  END IF;
